...
function AddressToLatLong($address)
{
    ...
    $url='https://maps.google.com/maps/api/geocode/json?address='.urlencode($address).'&sensor=false&key=AIzaSyDneupVRef7cUJk33W1XGPxR59PP3juOsk';
    $curl_return=curl_get($url);
    $obj=json_decode($curl_return);
    $_latitude = $obj->results[0]->geometry->location->lat;
    $_longitude = $obj->results[0]->geometry->location->lng;
...
$insert_data = mysqli_query($con,"INSERT INTO users(`phonenumber`,`name`,`email`,`password`,`role`,`created_at`,`updated_at`,`user_photo`) 
VALUES('$myphone','$name','$email','$phash','$role','$created_at','$updated_at','default.png')" ); 
if($insert_data) 
{ 
//return json message 
...
else if(isset($_POST['GetPaymentInformation'])) 
{
    ...
    $myObj->status = "success";
    $myObj->message = "Payment Reference for  ".$user_id."==".$user_token;
    $myObj->data = $rows;
...
else if(isset($_GET['TotalJobs'])) 
{
    ...
    $user_id =  getUserIDFromToken($con,$user_token);
     ...
     $select_stmt = mysqli_query($con,"SELECT * FROM delievery_logs where by_rider_id='$user_id' AND status='12' ");
    $rows = array();
    while($r = mysqli_fetch_assoc($select_stmt))
    {
    $rows[]=$r;
...
//Delete Vehicle
   ...
   $update_data = mysqli_query($con,"UPDATE users SET `email`='$email', `name`='$name', `user_photo`='$filename_img', `phonenumber`='$phonenumber' WHERE id='$user_id'    "); 
...
else if(isset($_POST['UpdateRiderProfile'])) 
        {
    ...
    $phonenumber =  mysqli_real_escape_string($con,$_POST['phonenumber']);
    $email =        mysqli_real_escape_string($con,$_POST['email']);
    $account_bank = mysqli_real_escape_string($con,$_POST['account_bank']);
   ...
   $update_data = mysqli_query($con,"UPDATE users SET `email`='$email', `name`='$name', `accountbank`='$account_bank', `accountname`='$account_name', `accountnumber`='$account_number', `phonenumber`='$phonenumber' WHERE id='$user_id'    "); 
   
 
    if($update_data)
    {
        $myObj->status = "success";
        $myObj->message = "Rider Profile updated !";
        $myObj->data = [];
 ...
 WHERE delievery_logs.by_rider_id = '$user_id' AND delievery_logs.status != '0' ");
   ...
   // echo json_encode($rows); 
        ...
        else if(isset($_POST['CheckCurrentRequests']))
        {
            ...
            {
               if( $row_s['status'] != 0 && $row_s['status'] !=12 && $row_s['status'] !=11 )
                {
        ...
        //get riders liscnes
            ...
            $array=($user_id);
            $select_stmt = mysqli_query($con,"SELECT * FROM delievery_logs where  `status`='0' AND  created_at >= CURDATE()  ");
            $rows = array();
            while($r = mysqli_fetch_assoc($select_stmt)) {
...
$expoPushToken = $_POST['expoPushToken']; 
...
while ($r = mysqli_fetch_array($select_stmt))
{
 ...
 /*    echo "1 - ".$r['name']." -".$r['id']." -".$r['approved']."-".$r['passportImage'];
   */  
   ...
   $current_id = $r['id'];
   //Update push token
   $update_push_token = mysqli_query($con,"UPDATE users SET `push_notification_token`='$expoPushToken'  WHERE id='$current_id'  ");
...
else if(isset($_POST['GET_KM_FROM_LAT_AND_LNG']))
{
   
    $pickup_address  =  mysqli_real_escape_string($con,$_POST['pickup_address']); 
    $destination_address  =  mysqli_real_escape_string($con,$_POST['delievery_destination']); 
 
    
    $address_dest = str_replace(" ", "+", $destination_address);
    $address_pickup = str_replace(" ", "+", $pickup_address);

    $destination_lat_lng = AddressToLatLong($address_dest);
    $pickup_lat_lng = AddressToLatLong($address_pickup);
  
 

    $DISTANCE_KM = calculateDistanceBetweenTwoPoints($pickup_lat_lng[0],$pickup_lat_lng[1],$destination_lat_lng[0],$destination_lat_lng[1],'KM',true,5);  
...
else if(isset($_POST['GET_TOTAL_TRIP_DISTANCE']))
{
...
else if(isset($_POST['Notify_Closest_Riders']))
{
    ...
    $longitude  =  mysqli_real_escape_string($con,$_POST['longitude']);  //customer's current longitude
    $latitude   =  mysqli_real_escape_string($con,$_POST['latitude']);  //customer's current latitude
    $ride_type   =  mysqli_real_escape_string($con,$_POST['ride_type']);  //customer's current ride_type
    ...
    $add_geo =   AddressToLatLong($pickup_address);

    $pickup_lat = $add_geo[0];
    $pickup_lng = $add_geo[1];


    //echo "Latitude: ".$pickup_lat." && ".$pickup_lng;
     ...
     while($select_wallet_amount = mysqli_fetch_array($select_wallet_amount))
     {
        ...
        $paystack_reference  = $select_wallet_amount['paystack_reference'];
     }
     ...
     $sum_kilometers = ((57)* ($add_kilometers)) + (200); //100 naira per kilometer // ( $add_kilometers / $total_i);
        

      
     $tranx = json_decode($paystack_reference);
     ...
     if($tranx->data->status =="success")
     {
        //start
         ...
         ( 3959 * acos( cos( radians($pickup_lat) ) * cos( radians( latitude ) ) 
         * cos( radians( longitude ) - radians($pickup_lng) ) + sin( radians($pickup_lat) ) 
         * sin(radians(latitude)) ) ) AS distance 
         FROM current_rider_positions 
         HAVING distance < 15
         ORDER BY distance 
        ...
        $last_insert_delievery_log = 0;
         ...
         $insert_delievery_log = mysqli_query($con,"INSERT INTO delievery_logs(`by_customer_id`,
          ...
          '0','0','$ride_type','0','$created_at','$updated_at', '0','$sum_kilometers' )
          ");
     ...
     $myvars = 'to=' . $ptoken . '&title=Connection Request&body=You have a new 
     ...
     }
     {
         ...
         if($found ==0)
         {
            echo "insufficient-fund -[]- ".$sum_kilometers."Paystack Card Data = ".$tranx->data->status;
         }
...
else if( isset($_POST['save_kiliometer']))
{
    ...
    $user_token = mysqli_real_escape_string($con,$_POST['user_token']); 
    $delievery_length  = mysqli_real_escape_string($con,$_POST['delievery_length']); 

    $get_existing_count = 0;

    $select_existing =    mysqli_query($con,"SELECT * FROM tempoary_kms  WHERE  `user_token`='$user_token' "); 
    while($r = mysqli_fetch_assoc($select_existing)) 
        { 
            $get_existing_count++;
        ...
        if($get_existing_count >= $delievery_length)
        {
         //do not insert
         echo "Trigger Cost"."-[]-".$kilometer;
        }
        else
        {
            $insert = mysqli_query($con,"INSERT INTO  tempoary_kms(`kilometer`,`user_token`) VALUES('$kilometer','$user_token')");
 
            echo "getting...xxx".$kilometer."-[]-".$user_token."-[]-".$get_existing_count;
        }
    ...
    if($update_)
    {
        ...
        //echo 1;
            ...
            while($row_select_stmt = mysqli_fetch_array($select_stmt))
            {
                ...
                //deduct money automatically from paystack. 
...
current_rider_positions.rider_id = users.id 
 WHERE delievery_logs.by_customer_id = '$user_id' AND delievery_logs.status != '0' ");
    ...
    while($row_select_ = mysqli_fetch_assoc($select_))
    {
            if($row_select_['status'] == 12 || $row_select_['status'] == '12' || $row_select_['status'] == '11'  )
            {
                continue;
            }
        $rows[]=$row_select_; 
